...
app.get('/api/livekit/token', async (req, res) => {
  const roomName = req.query.room || 'talkme-default-room';
  const participantName = req.query.participant || 'Guest_' + Math.floor(Math.random() * 1000);

  try {
    const token = await createToken(roomName, participantName);
    res.json({ token, room: roomName, participant: participantName });
  } catch (error) {
